/**
 * Mika & Co. — Automated Transactional Email Engine
...
export async function sendEmailReceipt(req: SendEmailReceiptRequest) {
  ...
  const htmlBody = `
    <div style="font-family: serif; background-color: #18120C; color: #FAF6EE; padding: 32px; border-radius: 24px; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #E8AB48; margin-bottom: 8px;">Mika & Co. — Luxury Dessert Boutique</h1>
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Mika & Co. <orders@mikaandco.com>",
          to: [to],
          subject: `Reservation Receipt #${orderId} — Mika & Co.`,
          html: htmlBody,
        }),
      });
