/**
 * La Miette Brownie — SMS Gateway Notification Engine
...
export const SMS_TEMPLATES = {
  orderConfirmed: (name: string, orderId: string, pickupTime: string) =>
    `La Miette Brownie: Hello ${name}! Your bakery order #${orderId} is confirmed for ${pickupTime} pickup. Total batch locked!`,

  orderInOven: (orderId: string) =>
    `La Miette Brownie: Great news! Your order #${orderId} is currently baking in our hearth oven. Smells heavenly!`,

  orderReady: (orderId: string, address: string) =>
    `La Miette Brownie: Fresh out! Your order #${orderId} is ready for pickup at ${address}. Please show this receipt.`,
};
